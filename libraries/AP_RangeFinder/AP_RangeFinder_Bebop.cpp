...
int AP_RangeFinder_Bebop::apply_averaging_filter(void)
{
...
int AP_RangeFinder_Bebop::search_local_maxima(void)
{
...
int AP_RangeFinder_Bebop::search_maximum_with_max_amplitude(void)
{
...
void AP_RangeFinder_Bebop::_loop(void)
{
    ...
    while(1) {
        ...
        if (apply_averaging_filter() < 0) {
            hal.console->printf(
                    "AR_RangeFinder_Bebop: could not apply averaging filter");
        }

        if (search_local_maxima() < 0) {
            hal.console->printf("Did not find any local maximum");
        }

        max_index = search_maximum_with_max_amplitude();
        if (max_index >= 0) {
            _altitude = (float)(max_index * P7_US_SOUND_SPEED) /
                (2 * (P7_US_DEFAULT_ADC_FREQ / _filter_average));
            _mode = _update_mode(_altitude);
        }
    }
}

void* AP_RangeFinder_Bebop::_thread_func(void* arg)
{
    AP_RangeFinder_Bebop *rfnd = (AP_RangeFinder_Bebop*) arg;
...
void AP_RangeFinder_Bebop::update(void)
{
    ...
    if (first_call) {
        ...
        if (ret != 0) {
            hal.console->printf(
                    "AP_RangeFinder_Bebop: pthread_attr_init failed\n");
            return;
        }
        pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED);
        pthread_attr_setschedpolicy(&attr, SCHED_FIFO);
        pthread_attr_setschedparam(&attr, &param);
        ret = pthread_create(&_thread, &attr,
                AP_RangeFinder_Bebop::_thread_func, this);
        first_call = false;
    }
...
{
    _spi = hal.spi->device(AP_HAL::SPIDevice_Bebop);
    if (_spi == NULL) {
