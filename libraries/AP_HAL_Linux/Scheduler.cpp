...
bool LinuxScheduler::register_timer_process(AP_HAL::MemberProc proc,
                                            uint8_t freq_div)
{
    if (freq_div <= 1) {
...
bool LinuxScheduler::_register_timesliced_proc(AP_HAL::MemberProc proc,
                                               uint8_t freq_div)
{
    unsigned int i, j;
...
void LinuxScheduler::_run_timers(bool called_from_timer_thread)
{
    ...
    for (i = 0; i < _num_timesliced_procs; i++) {
        if ((_timer_count + _timesliced_proc[i].timeslot)
    ...
    if (_max_freq_div != 0) {
        /* if there is a timesliced proc registered, round the max count to be
         * a number of freq div in order to simplify the tick count
         */
        _timer_count++;
        if (_timer_count / _max_freq_div == (0xffffffff / _max_freq_div)
                                           * _max_freq_div) {
            _timer_count = 0;
        }
    }
