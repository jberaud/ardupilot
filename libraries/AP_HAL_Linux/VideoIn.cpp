...
bool VideoIn::get_frame(Frame &frame)
{
    if (!_streaming) {
        if (!_set_streaming(true)) {
            hal.scheduler->panic("couldn't start streaming\n");
            return false;
        }
        _streaming = true;
    }
...
bool VideoIn::allocate_buffers(uint32_t nbufs)
{
    ...
    for (i=0; i < rb.count; i++) {
        ...
        case V4L2_MEMORY_USERPTR:
            ret = posix_memalign(&buffers[i].mem, getpagesize(),
                    buf.length);
            if (ret < 0) {
...
bool VideoIn::set_format(uint32_t width, uint32_t height, uint32_t format)
{
    ...
    _sizeimage = fmt.fmt.pix.sizeimage;
...
bool VideoIn::_dequeue_frame(Frame &frame)
{
    ...
    frame.pixelformat = _format;
    frame.bytesperline = _bytesperline;
    frame.sizeimage = _sizeimage;
